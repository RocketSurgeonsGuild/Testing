...
"
    ...
    [Test]
    [Arguments(DiagnosticSeverity.Error)]
    [Arguments(DiagnosticSeverity.Warning)]
    [Arguments(DiagnosticSeverity.Info)]
    [Arguments(DiagnosticSeverity.Hidden)]
    ...
    [Test]
    [MethodDataSource(nameof(GeneratorTestResultsCustomizerData))]
    public async Task Should_Support_Customizers(string name, GeneratorTestResultsCustomizer customizer)
    {
        var context = GeneratorTestContextBuilder
                     ...
                     .WithGenerator<MySourceGenerator>()
                     .Build();
        await Verify(context.GenerateAsync()).UseParameters(name);
    }
