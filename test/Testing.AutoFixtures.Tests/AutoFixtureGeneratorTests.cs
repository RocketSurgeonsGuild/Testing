...
public class AutoFixtureGeneratorTests
{
    ...
    public async Task GivenNoConstructor_WhenGenerate_ThenReportsDiagnostic(GeneratorTestContext context)
    {
        ...
        result
           .Results
           .Should()
           .Contain(pair => ImmutableArrayExtensions.All<Diagnostic>(pair.Value.Diagnostics, diagnostic => diagnostic.Id == Diagnostics.AutoFixture0001.Id));
    }
    ...
    public async Task GivenConstructorWithParameterArray_WhenGenerate_ThenReportsDiagnostic(GeneratorTestContext context)
    {
        ...
        result
           .Results
           .Should()
           .Contain(pair => ImmutableArrayExtensions.Any(pair.Value.Diagnostics, diagnostic => diagnostic.Id == Diagnostics.AutoFixture0002.Id));
    }
}
